...
# Model Deployment
The finalized models is persisted as a pickle file - which is then utilized in the backend Web Application developed in Flask microservice framework, exposed as Http based RestFul endpoints.

## Architecture
The following are the high level layout of the deployment architecture:
1. The web application can be invoked using browsers as well as mobile devices
2. The FrontEnd application is developed using Angular framework
3. FrontEnd web application is packaged as docker image and deployed in AWS EC2 instance
4. Frontend web application invokes the backend service api - which utilizes the ML Models embedded within Flask Web API
5. Backend Flask Web Application is deployed in separate AWS EC2 instance utilizing the docker image
6. Backend Flask Web Application is exposed as Http Restful endpoints for utilizing the two ML models
7. Interaction between frontend and backend utilizes Http Post methods with data exchange format as JSON

```{r da, out.width = "115%", echo=FALSE, fig.align='center', fig.cap="Deployment Architecture"}
include_graphics("../../artifacts/architecture.png")
```

## Docker
Docker architecture approach is utilized to deploy the web application two distinct layers.
Once the docker images are generated, same gets published within docker registry or with AWS ECR.
The images can be pulled from the image repository for deployement in AWS EC2 or local server as per needs.
