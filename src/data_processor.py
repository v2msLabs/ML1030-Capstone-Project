...
def rank(ranges, value, default_rank=0):
    ...
    for k, v in ranges.items():
        ...
        if l == 1 and value <= v[0]:
            return k
        elif l > 1 and value >= v[0] and value <= v[1]:
            return k
        elif i > 1 and l == 1 and value >= v[0]:
            return k
        i = i + 1
    return default_rank
...
"""
Process one observation (row)
"""


def process_row(row):
    row[2] = categorize(row[2], maps.map_col2)
    row[3] = categorize(row[3], maps.map_col3_5)
    row[4] = categorize(row[4], maps.map_col4)
    row[5] = categorize(row[5], maps.map_col3_5)
    row[6] = categorize(row[6], maps.map_col6_7_8)
    row[7] = categorize(row[7], maps.map_col6_7_8)
    row[8] = categorize(row[8], maps.map_col6_7_8)
    row[9] = categorize(row[9], maps.map_col9, default_category=2)
    row[10] = categorize(row[10], maps.map_col10)
    row[11] = categorize(row[11], maps.map_col11)
    row[12] = categorize(row[12], maps.map_col12)
    row[13] = categorize(row[13], maps.map_col13)
    ...
    row[25] = categorize(row[25], maps.map_col25)
    row[26] = categorize(row[26], maps.map_col26, default_category=2)
    row[27] = categorize(row[27], maps.map_col27_28_34_35)
    row[28] = categorize(row[28], maps.map_col27_28_34_35)
    row[29] = categorize(row[29], map=maps.map_col29)
    row[30] = categorize(row[30], maps.map_col30_31_32)
    row[31] = categorize(row[31], maps.map_col30_31_32)
    row[32] = categorize(row[32], maps.map_col30_31_32)
    row[33] = categorize(row[33], maps.map_col33)
    row[34] = categorize(row[34], maps.map_col27_28_34_35)
    row[35] = categorize(row[35], maps.map_col27_28_34_35)
    row[36] = categorize(row[36], maps.map_col36)
    ...
    row["credit_score_category"] = rank(dc.credit_ranking,row["credit_score"],default_rank=1)
    # lender score
    # risk
    ...
    row["lender_score"] = round((recipient_w + interest_w + collateral_w + frequency_w +
                                 duration_w + amount_w + collection_w + usage_w) * 1000)
    row["lender_score_category"] = rank(dc.credit_ranking,row["lender_score"],default_rank=1)
    return row
